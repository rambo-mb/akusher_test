...
export function Stats(props: { onBack: () => void }) {
  ...
  useEffect(() => {
    Promise.all([api.stats(), api.attempts(), api.statsWeak()])
      .then(([statsData, attemptsData, weakData]) => {
        setS(statsData);
        setAttempts(attemptsData.slice().reverse());
        setWeak(weakData);
      })
  ...
  return (
    <>
      <div className="header-row" style={{ display: "flex", alignItems: "center", marginBottom: 16 }}>
        ...
        <h2 style={{ flex: 1, textAlign: "center", margin: 0 }}>{t("stats.title")}</h2>
      ...
      {!loading && attempts.length > 0 && (
        <div className="card" style={{ padding: "20px 10px" }}>
          <div style={{ fontWeight: 600, marginBottom: 16, textAlign: "center" }}>{t("stats.chartTitle")}</div>
          <Chart data={attempts.map((a) => a.score)} noDataLabel={t("stats.noChart")} />
        </div>
      )}

      {!loading && s && (
        <div className="card">
          <Row label={t("stats.completed")} value={s.totalAttempts} />
          <Row label={t("stats.answered")} value={s.totalAnswered} />
          <Row label={t("stats.correct")} value={s.totalCorrect} />
          <Row label={t("stats.accuracy")} value={`${s.accuracy}%`} />
          <Row label={t("stats.best")} value={`${s.bestScore}%`} />
          <Row label={t("stats.mistakes")} value={s.mistakesCount} />
          <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0" }}>
            <span style={{ color: "var(--tg-hint)" }}>{t("stats.reminders")}</span>
            <label style={{ display: "flex", alignItems: "center", cursor: "pointer" }}>
              ...
              />
              <span>{s.remindersOn ? t("stats.on") : t("stats.off")}</span>
            </label>
          </div>
        </div>
      )}
      ...
      {!loading && weak.length > 0 && (
        <div className="card">
          <div style={{ fontWeight: 600, marginBottom: 12 }}>{t("stats.weakTitle")}</div>
          {weak.map((w) => (
            <div
              key={w.questionId}
              style={{
                padding: "8px 0",
                borderBottom: "1px solid color-mix(in srgb, var(--tg-hint) 20%, transparent)",
              }}
            >
              <div style={{ fontSize: 13, marginBottom: 4 }}>{w.stem}</div>
              <div style={{ fontSize: 11, color: "var(--tg-hint)" }}>
                {t("stats.wrongCount")}:{" "}
                <span style={{ color: "var(--red)", fontWeight: "bold" }}>{w.wrongCount}</span>{" "}
                | {t("stats.mastery")}: {w.box}/5
              </div>
            </div>
          ))}
        </div>
...
function Chart({ data, noDataLabel }: { data: number[]; noDataLabel: string }) {
  if (data.length < 2)
    return (
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="120" style={{ display: "block" }}>
      ...
      {data.map((val, i) => {
        ...
        const y = height - padding - (val / maxVal) * (height - 2 * padding);
        return (
          <circle key={i} cx={x} cy={y} r="4" fill="var(--tg-bg)" stroke="var(--tg-button)" strokeWidth="2" />
        );
      })}
    </svg>
  );
