...
export function History(props: { onBack: () => void, onOpenAttempt: (res: FinishResponse) => void }) {
  ...
  useEffect(() => {
    api.attempts().then(setAttempts).catch(e => setError((e as Error).message)).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", marginBottom: 16 }}>
        ...
        <h2 style={{ flex: 1, textAlign: "center", margin: 0 }}>📜 Tarix</h2>
        {!inTg && <div style={{ width: 80 }} />}
      </div>
      <div className="card">
        {attempts.length === 0 ? (
          <div className="review-note" style={{ textAlign: "center", padding: "20px 0" }}>Hali testlar yechilmagan.</div>
        ) : (
          attempts.map((a, i) => (
            <div key={a.id} onClick={() => openAttempt(a.id)} style={{ display: "flex", alignItems: "center", padding: "12px 0", borderBottom: i < attempts.length - 1 ? "1px solid color-mix(in srgb, var(--tg-hint) 20%, transparent)" : "none", cursor: "pointer" }}>
              <div style={{ flex: 1 }}>
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ fontWeight: "bold", fontSize: 16, color: a.score >= 80 ? "var(--green)" : a.score >= 60 ? "orange" : "var(--red)" }}>
                  {a.score}%
                </div>
                <div style={{ fontSize: 12, color: "var(--tg-hint)" }}>{a.correctCount} / {a.count}</div>
              </div>
            </div>
          ))
        )}
